...
def g_force(p1,p2):
    ...
    return force_vec
sun=sphere(pos=vector(0,0,0),radius=0.17,color=color.orange,mass=250,momentum=vector(0,0,0),make_trail=True)
earth=sphere(pos=vector(1.8,0,0),radius=0.05,color=vector(1,0.31,0.4),mass=7.25,momentum=vector(0,60,0),make_trail=True)
#p1=sphere(pos=vector(-2,0,0),radius=0.075,color=color.red,mass=2,momentum=vector(0,-35,0),make_trail=True)
#p2=sphere(pos=vector(1.5,0,0),radius=0.1,color=vector(0,0,0),mass=3,momentum=vector(0,40,0),make_trail=True)
dt=0.0001
t=0
while(True):
    rate(500)
